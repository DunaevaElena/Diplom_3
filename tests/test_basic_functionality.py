...
class TestBasicFunctionality:
    ...
    @allure.title('Проверка перехода по ссылке «Конструктор» в хедере')
    def test_click_link_constructor(self, header, index_page):
    ...
    @allure.title('Проверка перехода по клику на ссылку «Лента заказов» в хедере')
    def test_click_link_orders_feed(self, header, index_page, order_feed_page):
        index_page.open_index_page()
        header.click_by_link_orders_feed()
    ...
    @allure.title('Проверка появления всплывающего окно с деталями при клике на ингредиент')
    def test_pop_up_details_window_by_click_on_ingredient(self, index_page):
        ...
        with allure.step(f'Проверяем открылось ли окно с деталями ингредиента ({ingredient_name})'):
    ...
    def test_close_pop_up_details_window(self, index_page):
        index_page.open_index_page()
    ...
    @allure.title('Проверка, что авторизированный пользователь может оформить заказ')
    def test_autorized_user_can_create_order(self, user, logged, index_page):
